...
>
  <div>
    <div class="font-semibold">{prism.id || $translate('app.labels.prisms')}</div>

    {#if prism.outlay_factor}
      <div class="w-full text-xs italic truncate whitespace-nowrap pr-1">Outlay: {prism.outlay_factor * 100}%</div>
    {/if}
  ...
  <div class="flex items-center ml-4 h-full">
    <div>
      ...
      >
        ...
        />
        <div>Member count: {prism.prism_members.length}</div>
      </div>
